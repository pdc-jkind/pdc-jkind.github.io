  ...
  try {
    ...
    if (encodedData) {
      try {
        ...
        // Jika error parsing, gunakan object kosong untuk Job Costing 1/DO atau array kosong untuk Job Costing 2
        parsedData = (rootCollection === 'Job Costing 1' || rootCollection === 'DO') ? {} : [];
      }
    } else {
...
async function processData(pdcName, batch, vehiclesData, parsedData, rootCollection, kodeGudang, keterangan, mode) {
  try {
    ...
    if (rootCollection === 'Job Costing 1') {
      ...
      console.log('Processing DO data');

      // Format untuk DO - menggunakan struktur seperti Job Costing 1
      preparedData = [{
        name: vehiclesData, // nama lokasi kendaraan
        productData: parsedData || {}, // data product dari parameter data (client: code)
        describe: keterangan || ""
      }];
    }
    else {
